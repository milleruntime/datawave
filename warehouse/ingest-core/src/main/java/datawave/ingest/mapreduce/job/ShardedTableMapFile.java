...
public class ShardedTableMapFile {
    ...
    public static void validateShardIdLocations(Configuration conf, String tableName, int daysToVerify, Map<Text,String> shardIdToLocation) {
        ...
        for (int daysAgo = 0; daysAgo <= daysToVerify; daysAgo++) {
            ...
            boolean shardsExist = shardsExistForDate(shardIdToLocation.keySet(), datePrefix, expectedNumberOfShards);
            if (!shardsExist) {
     ...
     */
    private static boolean shardsExistForDate(Collection<Text> locations, String datePrefix, int expectedNumberOfShards) {
        int count = 0;
        byte[] prefixBytes = datePrefix.getBytes();
        for (Text key : locations) {
