...
AC_CORE_JAR=`ls $WAREHOUSE_ACCUMULO_LIB/accumulo-core*.jar | sort | head -1`
AC_SERVER_JAR=`ls $WAREHOUSE_ACCUMULO_LIB/accumulo-server-base*.jar | sort | head -1`
AC_START_JAR=`ls $WAREHOUSE_ACCUMULO_LIB/accumulo-start*.jar | sort | head -1`
AC_TRACE_JAR=`ls $WAREHOUSE_ACCUMULO_LIB/accumulo-tracer*.jar | sort | head -1`
